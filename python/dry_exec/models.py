...
class ByteDelta(BaseModel):
    ...
    offset: int = Field(description="Relative byte offset from start of mapped region or file")
...
class PageMutation(BaseModel):
    ...
    deltas: List[ByteDelta] = Field(default_factory=list, description="List of discrete byte mutations")
...
class FsMutation(BaseModel):
    ...
    mutation_type: str = Field(description="Classification: created, modified, or deleted")
    ...
    deltas: List[ByteDelta] = Field(default_factory=list, description="Byte deltas for modified files")
...
class InterceptedRequest(BaseModel):
    ...
    headers: Dict[str, str] = Field(default_factory=dict, description="Captured request headers")
    request_body: bytes = Field(default=b"", description="Captured outbound request body")
    response_status: int = Field(description="HTTP status code returned by transparent proxy")
    response_body: bytes = Field(default=b"", description="Mock response payload returned by transparent proxy")
...
class StateDelta(BaseModel):
    ...
    duration_nanos: int = Field(description="State delta computation latency in nanoseconds")
